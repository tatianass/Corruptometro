...
tre_sagres_jul <- read.csv('TRE_Sagres_Resp_Eleito.csv', encoding = "UTF-8")
tre_sagres_n_jul <- read.csv('TRE_Sagres_Eleit_Idon.csv', encoding = "UTF-8")

tre_sagres_jul$class <- "Julgados"
tre_sagres_n_jul$class <- "Não julgados"


tre_sagres_jul <- select(tre_sagres_jul,-DECISÃO, -RES..DECISÃO.PODER.LEGISLATIVO, -ITEM, -PROCESSO, -SUBCATEGORIA, -RESPONSÃVEL, -CPF)
...
write.table(tre_sagres, "tre_sagres_unificadoBase.csv", sep=";", row.names = F, quote = F, fileEncoding = "UTF-8")
